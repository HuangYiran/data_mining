...
* transactions shoulb be iterator of iterator object, e.g list of list: [[1,2,3,4,5],[2,3,4,5]]
* when include_support set True, find_frequent_itemsets will returns two value(include the support), otherwise return only one
* we only jield the result(itemset and support) in the find_frequent_itemsets, not return, which means we need a for loop to extract the result, usually we do it as shown below:
    result = []
